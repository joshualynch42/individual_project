...
model = PPO.load("D:/Users/Josh/github/individual_project/simulation/sim_agents/PPO_agent_after", env=env_p)
...
if environment == 'arrow':
    current_letter = 'DOWN'
    goal_letter = 'DOWN'
    ####################[ nothing, fire, right, left, right, left]
    action_to_key_arr = ['LEFT', 'LEFT', 'RIGHT', 'DOWN', 'RIGHT', 'DOWN']
    key_to_action_dict = {'LEFT': [1], 'RIGHT': [4], 'DOWN': [5]}
    ...
    env_r = phys_discrete_alphabet_env_pong(current_letter, goal_letter)
...
# for i in range(500):
    ...
    current_state = env_r.reset(current_letter, goal_letter)
    # done = False
    # while not done: commented out to force robot into making a single step
    action_r = agent.act(current_state) # predict robot action
    new_state, reward, done, _ = env_r.step(action_r, steps=0) # execute action
